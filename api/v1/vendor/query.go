...
func (r *vendorQuery) GetMaterialByID(id int64) (*MaterialResponse, error) {
	var material MaterialResponse
	err := r.conn.Get(&material, "SELECT id, name, status FROM materials WHERE id = ? AND status > 0 ", id)
	return &material, err
}

func (r *vendorQuery) GetMaterialCount(filter MaterialFilter) (int, error) {
	where, args := []string{"status > 0"}, []interface{}{}
	if v := filter.Name; v != "" {
		where, args = append(where, "name like ?"), append(args, "%"+v+"%")
	}
...
func (r *vendorQuery) GetMaterialList(filter MaterialFilter) (*[]MaterialResponse, error) {
	where, args := []string{"status > 0"}, []interface{}{}
	if v := filter.Name; v != "" {
		where, args = append(where, "name like ?"), append(args, "%"+v+"%")
	}
