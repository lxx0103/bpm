...
func (r *vendorRepository) CreateMaterial(info MaterialNew) error {
	_, err := r.tx.Exec(`
...
func (r *vendorRepository) UpdateMaterial(id int64, info MaterialNew) error {
	_, err := r.tx.Exec(`
...
func (r *vendorRepository) GetMaterialByID(id int64) (*MaterialResponse, error) {
...
func (r *vendorRepository) CheckMaterialNameExist(name string, selfID int64) (int, error) {
	var res int
...
func (r *vendorRepository) CheckMaterialActive(materialID int64) (int, error) {
	// var res int
	// row := r.tx.QueryRow(`SELECT count(1) FROM material_vendors WHERE material_id = ? AND status > 0 LIMIT 1`, materialID)
	// err := row.Scan(&res)
	// if err != nil {
	// 	return 0, err
	// }
...
func (r *vendorRepository) DeleteMaterial(id int64, byUser string) error {
	_, err := r.tx.Exec(`
		Update materials SET 
		status = ?,
		updated = ?,
