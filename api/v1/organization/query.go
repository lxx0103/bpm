...
func (r *organizationQuery) GetOrganizationByID(id int64) (*OrganizationResponse, error) {
	...
	err := r.conn.Get(&organization, "SELECT id, name, logo, logo2, description, phone, contact, address, city, type, user_limit, status FROM organizations WHERE id = ? ", id)
	if err != nil {
		return nil, err
	}
...
func (r *organizationQuery) GetOrganizationList(filter OrganizationFilter) (*[]OrganizationResponse, error) {
	...
	err := r.conn.Select(&organizations, `
		SELECT id, name, logo, logo2, description, phone, contact, address, city, type, user_limit, status
		FROM organizations 
