...
func (r *eventQuery) GetAssignedEventByID(id int64, status string) (*MyEvent, error) {
	var event MyEvent
	sql := "SELECT e.id, e.project_id, p.name as project_name, e.name, e.complete_user, e.complete_time, e.audit_user, e.audit_time, e.audit_content, e.need_checkin, e.sort, e.status FROM events e LEFT JOIN projects p ON p.id = e.project_id WHERE e.id = ?"
	if status == "all" {
...
func (r *eventQuery) GetProjectEvent(filter MyEventFilter) (*[]MyEvent, error) {
	var event []MyEvent
	sql := "SELECT e.id, e.project_id, p.name as project_name, e.name, e.complete_user, e.complete_time, e.audit_user, e.audit_time, e.audit_content, e.need_checkin, e.sort, e.status FROM events e LEFT JOIN projects p ON p.id = e.project_id WHERE e.project_id = ?  "
